(() => {
  ...
  function App() {
        ...
        try {
          ...
          if (r && r.value) {
            ...
            if (s.startDate) {
              ...
              const todayCopy = /* @__PURE__ */ new Date();
              todayCopy.setHours(0, 0, 0, 0);
              const todayMon = getMonday(todayCopy);
              const oldWeek = s.activeWeek || 1;
              const inferredStart = addDays(todayMon, -7 * (oldWeek - 1));
              setStartDate(dateToISO(inferredStart));
            }
          } else {
    ...
    }, [distance, raceDate, startDate, lthr, ltpStr, completed, logs, terrainOverrides, activeWeek, loaded]);
    ...
    const { totalWeeks, currentWeekFromDate, racePassed, startMonday } = useMemo(() => {
      ...
      if (!start) {
        const startMon2 = addDays(raceMonday, -7 * (preset.idealWeeks - 1));
        return {
          totalWeeks: preset.idealWeeks,
          currentWeekFromDate: 1,
          racePassed: false,
          startMonday: startMon2
        };
      }
      const startMon = getMonday(start);
      const total = weeksBetween(startMon, raceMonday) + 1;
      if (raceMonday.getTime() < todayMonday.getTime()) {
        return { totalWeeks: Math.max(4, total), currentWeekFromDate: Math.max(4, total), racePassed: true, startMonday: startMon };
      }
      const weekIdx = weeksBetween(startMon, todayMonday) + 1;
      return {
        totalWeeks: Math.max(4, total),
        currentWeekFromDate: Math.max(1, Math.min(Math.max(4, total), weekIdx)),
        racePassed: false,
        startMonday: startMon
      };
    }, [raceDate, startDate, preset.idealWeeks, today]);
    useEffect(() => {
      if (loaded && currentWeekFromDate >= 1 && currentWeekFromDate <= totalWeeks) {
        setActiveWeek(currentWeekFromDate);
      }
    }, [loaded, startDate, raceDate, distance]);
    const week = useMemo(
    ...
    const weekDates = useMemo(() => {
      ...
      const activeWeekMonday = addDays(startMonday, 7 * (activeWeek - 1));
      const dates = {};
      DAY_NAMES.forEach((d, i) => {
        dates[d] = addDays(activeWeekMonday, i);
      });
      return dates;
    }, [startMonday, activeWeek]);
    ...
    ))), /* @__PURE__ */ React.createElement("div", { style: { marginBottom: "12px" } }, /* @__PURE__ */ React.createElement("label", { style: { fontSize: "12px", color: "var(--color-text-secondary)", display: "block", marginBottom: "4px" } }, "Startdatum schema", /* @__PURE__ */ React.createElement("span", { style: { fontSize: "10px", color: "var(--color-text-tertiary)", marginLeft: "6px" } }, "(maandag van week 1)")), /* @__PURE__ */ React.createElement(
      "input",
      {
        type: "date",
        value: startDate || "",
        onChange: (e) => {
          ...
          if (v) {
            const d = parseDate(v);
            if (d) setStartDate(dateToISO(getMonday(d)));
          }
        },
        style: { width: "100%" }
      }
